...
def calc_tropo_temp(ta):
    ...
    T_interp = sp.interpolate.CubicSpline(plev[::-1], ta.isel{"pressure_level":slice(None, None, -1)})
...
def calc_tropo_gridded(vert_temp):
    tropopause_gridded = xr.apply_ufunc(calc_tropo_temp, vert_temp, input_core_dims=['pressure_level'], output_core_dims=[],
                                        parallelize=True, dask='parallelize')
...
def main():
    ...
    surf_data['trp_temp'] = calc_tropo_gridded(vert_temp)
